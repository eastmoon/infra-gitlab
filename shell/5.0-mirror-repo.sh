...
## 鏡像專案：將來源專案的原碼樹從自身的主線分支 ( master ) 或特定分支 ( feature/<branch name>) 鏡像到目標專案的主線分支 ( master )
#!/bin/bash
set -e
# Include library
source ./src/utils.sh
...
SOURCE_REPO=( DEMO ${SOURCE_REPO_NAME} master )
TARGET_REPO=( DEMO ${TARGET_REPO_NAME} master )
