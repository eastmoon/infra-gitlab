...
:cli-help (
    ...
    echo      check             Check gitlab status.
    echo      backup            Runing gitlab backup command.
    echo      restore           Runing gitlab restore command.
    echo.
...
:cli-into (
    ...
    docker exec -ti demo_service_gitlab_%PROJECT_NAME% bash
...
:: ------------------- Command "check" mathod -------------------

:cli-check (
    docker exec -ti demo_service_gitlab_%PROJECT_NAME% gitlab-rake gitlab:check SANITIZE=true
    goto end
)
...
:: ------------------- Command "backup" mathod -------------------

:cli-backup (
    docker exec -t demo_service_gitlab_%PROJECT_NAME% gitlab-backup create
    goto end
)
...
:cli-backup-help (
    echo Runing gitlab buckup command.
    goto end
)

:: ------------------- Command "restore" mathod -------------------

:cli-restore (
    echo ^> Stop the processes that are connected to the database
    docker exec -it demo_service_gitlab_%PROJECT_NAME% gitlab-ctl stop unicorn
    docker exec -it demo_service_gitlab_%PROJECT_NAME% gitlab-ctl stop puma
    docker exec -it demo_service_gitlab_%PROJECT_NAME% gitlab-ctl stop sidekiq

    echo ^> Verify that the processes are all down before continuing
    docker exec -it demo_service_gitlab_%PROJECT_NAME% gitlab-ctl status

    echo ^> Run the restore, by
    ::docker exec -it demo_service_gitlab_%PROJECT_NAME% gitlab-backup restore BACKUP=<least-gitlab-backup>

    echo ^> Restart the GitLab container
    docker restart demo_service_gitlab_%PROJECT_NAME%
...
:cli-restore-help (
    echo Runing gitlab restore command.
    goto end
)
